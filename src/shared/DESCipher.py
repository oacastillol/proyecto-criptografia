# Modulo encargado de generar las llaves, tomando como partida la llave
# dada por el usuario.
from .keyGenDES import keyGenDES
...
# tabla de expancion para manejo de parte derecha de mensaje y llave,
# inicio funcion f
expancion = [
    31, 0, 1, 2, 3, 4,
    3, 4, 5, 6, 7, 8,
...
# OUT -> result: lista de valores  de los datos ingresados, con espacios l
# definido por el largo del data
...
def bitsDeString(datos):
    ...
    for x in datos:
        i = 7
        while i >= 0:
            # por cada unicode de datos verifica si el AND de tamaño y posicion
            # es = 0 y asigna el valor correspondiente
            if x & (1 << i) != 0:
                result[pos] = 1
            else:
                result[pos] = 0
...
def stringDeBits(datos):
    ...
    c = 0
    while pos < len(datos):
        # corrimiento de bits para que el string sea conformado como bits y
        # luego con la cantidad suficiente (8) se agrega a resultado
        c += datos[pos] << (7 - (pos % 8))
        if (pos % 8) == 7:
            result.append(c)
            c = 0
...
# OUT-> lista de datos tratados para que el bloque quede organizado como
# la tabla dada
...
# OUT -> result: resultado de manejo del mensaje con llave dada y tipo
# ingresado
def cipher(msg, key, type):
    ...
    # tratado del mensaje para algoritmo, resultado en bits de la primera
    # permutacion IP
    mB = bitsDeString(msg)
    mB = permutar(ip, mB)
    ...
    # verificacion de proceso cifrado/descifrado (diferencia entre corrimiento
    # de llaves usadas)
    if type == "cipher":
    ...
    while i < 16:
        ...
        while k < 8:
            ...
            # conversion del valor entero v a bits, primero mirando and con
            # 1000 y luego corrimiento necesario
            Bn[pos] = (v & 8) >> 3
            Bn[pos + 1] = (v & 4) >> 2
            Bn[pos + 2] = (v & 2) >> 1
            Bn[pos + 3] = v & 1
...
# OUT -> c: string unificado de particiones tratadas del mensaje,
# resultado de mensaje cifrado o descifrado
...
def DES(msg, key, type):
    ...
    # inicializacion de string respuesta y temporal para espacio de 8
    # caracteres por cifrado
    c = []
    temp = ""

    # division del mensaje en espacios de 8 caracteres, guardado de
    # respectivos cifrados
    while len(msg) > 7:
        temp = msg[:8]
        res = cipher(temp, key, type)
        c.extend(res)
        msg = msg[8:]
    # si el mensaje no es multiplo de 8 rellena con x al final y ejecuta
    # ultima iteracion de msg para encontrar cifrado
    if len(msg) > 0:
        while len(msg) < 8:
            msg.append(10)
